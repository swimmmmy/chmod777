...
- `mavlink start -x -u 14555 -r 4000000 -t 127.0.0.1 -o 14550` : MAVLink를 새로 시작하는 명령.
  ...
  PX4와 QGC가 같은 14550 포트를 동시에 열려고(bind) 하면 충돌이 나서, PX4는 14555에서 보내고 QGC는 14550에서 받도록 포트를 나눠야 한다!
...
(근데 자동으로 하는게 좋음! 안 그러면 충돌날 가능성 높음)
...
처음에 `docker run -p 14550:14550/udp` 방식으로 시도했을 때는 QGC가 계속 "Disconnected" 상태였다...
...
### 드론 두 대 동시 실행은 무리였음 (학교pc는 가능할지도?)
...
두 번째 드론까지는 실행됐지만, 8코어/16GB로도 감당이 안 될 만큼 시스템 전체가 심하게 느려졌다. 
